...
export class EmbeddingController {
    ...
    async promptCompletionComparison(
        prompt: string,
        res: express.Response
    ) {
        const promptEmbedding = await this.getEmbedding(prompt);
        
        const promptCompletionEmbeddingArray = await this.getEmbedding(
            syllabusPrompts.map(pc => pc.prompt)
        );
        ...
        // if(comparisons[0].similarity < 0.9) {
        //     comparisons[0].response = "I don't have an answer for that question.";
        // }

        res.status(200).json(comparisons[0]);
    }

    async queryRelevantText(
        prompt: string,
        res: express.Response
    ) {
        ...
        const query: ChatCompletionRequestMessage = {
            role: "user",
            content: `Answer the following question using only the provided text.  
                        Question: ${prompt}, Text: ${comparisons[0].response}`
        }
